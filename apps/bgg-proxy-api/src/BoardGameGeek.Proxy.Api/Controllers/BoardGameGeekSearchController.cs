...
public class BoardGameGeekSearchController (HttpClient httpClient, IMapper mapper) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<GameSearchResponse>>> Get(string searchTerm, [FromQuery] bool searchById = false)
    {
        ...
        return Ok(mapper.Map<List<GameSearchResponse>>(bggThingResponse.Items));
    }
}
